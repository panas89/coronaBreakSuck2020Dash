...
meaningfull_filenames = ['Top 5-10\% High impact papers','Semantic scholar COVID-19 papers',
                         'COVID-19 Clinical Trials','Top 5\% High impact papers']
...
def getPubScatter(df,x,y,hover_name):
    ...
    fig = px.scatter(
                    df, x=x, y=y,hover_name=hover_name
                    )
    fig.update_layout(
...
kws = [data_class_subclass[list(data_class_subclass.keys())[1]][20],data_class_subclass[list(data_class_subclass.keys())[1]][2]]
...
def getMult_KW_scatter_plot(df,kw_interest):
    ...
    df_new_p = rvis.preprocess_for_multiple_kw_visualization(df, kw_interest=kw_interest)
...
app.layout = html.Div(children=[
    ...
    dcc.Dropdown(
        options=[{'label': name, 'value': filename} for name, filename in zip(meaningfull_filenames,filenames)
                    ],
        value= filenames[0],
        multi= False,
        id='dd-files'
    ...
    dcc.Dropdown(
        options=[{'label': class_sub, 'value': class_sub} for class_sub in list(data_class_subclass.keys())
                    ],
    ...
    dcc.Dropdown(
        options=[{'label': kw, 'value': kw} for kw in data_class_subclass[list(data_class_subclass.keys())[0]]
                    ],
...
def update_output(class_sub_classes,filename):
    ...
    return kws_dict, kws, getMult_KW_scatter_plot(df_new,kw_interest=kws),getPubScatter(df_new, x='publish_time', 
                                                                                        y='probability', hover_name='keyword')
